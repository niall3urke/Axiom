...
using Axiom.Controls.Switch;

namespace Axiom.Controls.Checkbox
{
    class CheckboxLogic : LogicBase, IAppearanceProperties
    {
        ...
        private readonly TickDrawable _check;

        // ===================
        // ===== Constructors
        // ===================

        public CheckboxLogic() : base()
        {
            _box = new FocusableBackgroundDrawable();

            _check = new TickDrawable();
        }
        ...
        {
            _check.ForegroundColor = ForegroundColor;

            _check.Height = GetCheckHeight();

            _check.Width = GetCheckWidth();
        ...
        private int GetBoxHeight()
        {
            ...
            // The background drawable reduces the height/width because it 
            // assumes it's covering the entire area of a control (which 
            // have their w/h reduced by 1px). Hence +1 px.
            return height + (2 * (int)_box.FocusBorderWidth) + 1;
        }
